# build-imageconverter.ps1
# Script di automazione per la creazione di pacchetti zip
# di ImageConverter (CLI) e ImageConverterGUI per win-x64, linux-x64, osx-arm64.
...
$projects      = @(
    @{ Name = "ImageConverter";    Csproj = "ImageConverter\ImageConverter.csproj" }
    @{ Name = "ImageConverterGUI"; Csproj = "ImageConverterGUI\ImageConverterGUI.csproj" }
)
...
foreach ($rid in $runtimes) {
    $ridPublishDir = Join-Path $publishRoot $rid
    $zipName = "ImageConverter-$rid.zip"
...
# ── Riepilogo ─────────────────────────────────────────────────────
Write-Host "`n================================================" -ForegroundColor Green
Write-Host " Pacchetti ImageConverter generati con successo!" -ForegroundColor Green
Write-Host "================================================" -ForegroundColor Green
Write-Host " Cartella di output: $outputDir" -ForegroundColor Green
foreach ($rid in $runtimes) {
    $zipPath = Join-Path $outputDir "ImageConverter-$rid.zip"
    if (Test-Path $zipPath) {
        $fileInfo = Get-Item $zipPath
        Write-Host "  - ImageConverter-$rid.zip ($([math]::Round($fileInfo.Length / 1MB, 2)) MB)" -ForegroundColor Green
    }
}
Write-Host "========================================================" -ForegroundColor Green
